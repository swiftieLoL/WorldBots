...
namespace Helper
{
    ...
    {
    public:
        static G3D::Vector3 CalculateSidestepPoint(Player* bot, float angleOffset, float distance = 4.0f)
        {
            ...
            float evadX = curX + distance * std::cos(o + angleOffset);
            float evadY = curY + distance * std::sin(o + angleOffset);
            float evadZ = curZ;
